...
const debug = require('debug')('lbproxy-socks'); // module specific events
  ...
  constructor(options) {
    ...
    this.server = net.createServer(socket => {
      ...
      function authenticate(buffer) {
        ...
        binary.stream(buffer).word8('ver').word8('ulen').buffer('uname', 'ulen').word8('plen').buffer('passwd', 'plen').tap(args => {
          ...
          self.options.authenticate(args.uname.toString(), args.passwd.toString(), socket, authDomain.intercept(() => {
            ...
            responseBuffer[1] = RFC_1929_REPLIES.SUCCEEDED; // respond then listen for cmd and dst info
      ...
      function connect(buffer) {
        ...
        binaryStream.word8('ver').word8('cmd').word8('rsv').word8('atyp').tap(args => {
          // capture the raw buffer
          args.requestBuffer = buffer; // verify version is appropriate
          ...
          if (args.atyp === RFC_1928_ATYP.IPV4) {
            ...
            binaryStream.word8('addr.size').buffer('addr.buf', 'addr.size').tap(args => {
              args.dst.addr = args.addr.buf.toString();
            }); // ipv6
          } else if (args.atyp === RFC_1928_ATYP.IPV6) {
            binaryStream.word32be('addr.a').word32be('addr.b').word32be('addr.c').word32be('addr.d').tap(args => {
        ...
        }).word16bu('dst.port').tap(args => {
          if (args.cmd === RFC_1928_COMMANDS.CONNECT) {
            let connectionFilter = self.options.connectionFilter,
                connectionFilterDomain = domain.create(); // if no connection filter is provided, stub one
            ...
            }, // origin
            {
              ...
              SocksClient.createConnection(self.buildSocksOptions(args.dst.addr, args.dst.port), (err, info) => {
                if (!err) {
                  ...
                  responseBuffer[1] = RFC_1928_REPLIES.SUCCEEDED; // write acknowledgement to client...
      ...
      function handshake(buffer) {
        binary.stream(buffer).word8('ver').word8('nmethods').buffer('methods', 'nmethods').tap(args => {
          ...
          responseBuffer[0] = RFC_1928_VERSION;
          responseBuffer[1] = RFC_1928_METHODS.NO_AUTHENTICATION_REQUIRED; // check for basic auth configuration
